    ...
    <main>
        ...
        <div class="review_header">
            <div class="align_type">
                <span>정렬기준</span>
                <button id="align_default" data-align="default">번호 순</button>
                <button id="align_report" data-align="report">누적 신고 순</button>
                <button id="align_date" data-align="date">날짜순</button>
            </div>
            <div class="blind_status">
                <span>블라인드 여부</span>
                <button id="all" data-blind="all">모든 리뷰</button>
                <button id="blind" data-blind="blind">블라인드 된 리뷰</button>
                <button id="not_blind" data-blind="not_blind">정상 리뷰</button>
            </div>
                    ...
                    <tr>
                        ...
                        <td>
                            <c:if test="${item.ri_status != 1}">
                                <button class="status_btn review_blind" data-seq="${item.ri_seq}" data-status="1">블라인드</button>
                            </c:if>
                            <c:if test="${item.ri_status == 1}">
                                <button class="status_btn review_delete_cancel" data-seq="${item.ri_seq}" data-status="0">블라인드 해제</button>
                            </c:if>
                            <c:if test="${item.ri_status != 2}">
                                <button class="status_btn review_blind" data-seq="${item.ri_seq}" data-status="2">삭제</button>
                            </c:if>
                            <c:if test="${item.ri_status == 2}">
