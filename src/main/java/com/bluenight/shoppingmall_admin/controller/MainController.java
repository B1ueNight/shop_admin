...
import com.bluenight.shoppingmall_admin.mapper.MemberMapper;
import com.bluenight.shoppingmall_admin.mapper.SellerMapper;
...
public class MainController {
    ...
    public String getMain(HttpSession session) {
        //로그인 시도 여부를 검사하기 위해 세션에 저장된 값 try_login을 가져온다
        Boolean try_login = (Boolean)session.getAttribute("try_login");
        // 가져온 값이 null 또는 false라면 로그인 시도 없이 새로고침
    ...
    // restcontroller - qequestbodt에 입력 데이터를 같이 실어 보낸다.
    // controller - formdata로 전송되기 때문에 페이로드 방식.(body쓰지 않음)
    // model은 해당 페이지 안에서만 쓸 수 있는 local(지역) 변수
    // session은 해당 서비스 전체에서 사용 할 수 있는 global(전역) 변수
    @PostMapping("/login")
    public String getLogin(LoginVO data, HttpSession session) {
        ...
        session.setAttribute("login_msg", "아이디 혹은 비밀번호 오류 입니다.");
        return "redirect:/";
        // return "/index";
    }
    @GetMapping("/logout")
    ...
    public String postAdminLogin(LoginVO admin, HttpSession session){
        ...
        session.setAttribute("admin_status", "failed");
        session.setAttribute("admin_msg", "아이디 또는 비밀번호 오류 입니다.");
        return "redirect:/admin";         
    }
